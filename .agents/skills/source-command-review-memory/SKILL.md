---
name: "source-command-review-memory"
...
# source-command-review-memory

Use this skill when the user asks to run the migrated source command `review_memory`.

## Command Template

You are running a memory hygiene audit for the current Codex session. Memories at `<memoryDir>/` (the auto-memory store described in your system prompt — the path varies per machine and per project) accumulate over time and some claims drift. This command surfaces them all so the user can prune.
