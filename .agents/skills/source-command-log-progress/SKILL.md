---
name: "source-command-log-progress"
description: "Manually append a branch-log entry for the current branch summarizing the most recent prompt's work."
---

# source-command-log-progress

Use this skill when the user asks to run the migrated source command `log_progress`.

## Command Template

You are writing one entry into the branch log. This is the manual fallback for the automatic branch-log protocol described in `.Codex/AGENTS.md` — use it when the user explicitly wants an entry recorded now.
