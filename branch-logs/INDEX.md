...
| main | (none) | 2026-07-13 14:00 | active — v0.6.2 GEPUBLICEERD + gepusht (17 pkgs; `@luckystack/cron` als wizard-opt-in `--cron` [dependency-only, byte-parity met `add cron`] + upgrade-runbook dekt nu nieuw-optioneel-package via `luckystack add`). Voorheen: v0.6.1 (mikro-fix + 7 devkit/core-fixes + `update --app` [ADR 0025]) | 30 |
