...
| main | (none) | 2026-07-15 | active — onderzoeksronde (3 agents, read-only): findings-migratie naar `docs/findings/2026-07-02-security/` + 2 nieuwe grootboeken. **LIVE bug gevonden**: wire-type-leugen `Date`→`string` in `session_v1` (T1/T2 HIGH, crasht al server-side) + `DEPTH_LIMIT` is de corruptie-TRIGGER (T4) → aanbeveling wire-projectie. Bun: env-auto-load keert precedence om (B1 HIGH) + `redis.ts:40` REDIS_HOST-snapshot-bug (B9 HIGH). Scope: npm+node+bun. Voorheen: v0.6.7 GEPUBLICEERD (`luckystack upgrade` + `ai:changelog-check` + Findings-protocol) | 37 |
