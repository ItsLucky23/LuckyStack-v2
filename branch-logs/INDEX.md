...
| main | (none) | 2026-07-16 | active — **Benchmark npm+node vs bun**: bun snellere runtime (json 1.60×, sort 1.50×, TS-exec ~2.9× vs tsx), npm snellere installer op Windows (npm ci 13.6s vs bun 43.2s, ~3.2×, gecaveat). 2 meetfouten gevangen (npm ci fast-fail, --offline-asymmetrie). Eerder vandaag: **C-04 gefixt** (secret-manager-pointers bevroren in config.ts op module-load; runtime bewezen, fix via ADR-0026-channel, repo+template) → alle findings-folders 0-open. **Bun-matrix her-geverifieerd: 4/4 verdaccio-cellen groen** (npm+node, bun+node, bun+bun, npm+bun). **ORM-projectie op DIEPTE getest** (nieuw 3-lagen fixture: 179 chars, createdAt→string op elk niveau). Eerder deze branch: **WS-upgrade dóór de router → 4 defecten** waaronder B17 **CRITICAL** (forwarded 101 verloor `Connection: Upgrade`, nóóit een WS door de router, stuk sinds 19 juni, **uitgeleverd in v0.6.7**), B22 router-shutdown hing, B19 Bun kan geen WS proxyen (upstream, router weigert luid). Voorheen: v0.6.7 GEPUBLICEERD | 40 |
