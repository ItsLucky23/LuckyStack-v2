...
| main | (none) | 2026-07-14 15:25 | active — v0.6.7 GEPUBLICEERD + gepusht (17 pkgs): `luckystack upgrade` (read-only plan-generator) + `ai:changelog-check` (report-only gate) + Findings & Dated-Docs Protocol (docs/findings/<date>-*/ + status-grootboek) + upgrade-docs altijd-bereikbaar (node_modules). Voorheen: v0.6.6 (cleanup), v0.6.5 (Redis secret-pointer echte fix [ADR 0026]) | 36 |
