...
| main | (none) | 2026-09-03 13:55 | active — **v0.10.0 gepubliceerd** (tag op `39942d2`; 17/17 op npm geverifieerd via directe registry-fetch, tarball-grep bevestigt sync-reorder, adapter-key en lintregel; e2e-verdaccio 9/9 vóór de tag; `fast-uri` 3.1.5→3.1.7 vooraf gebumpt omdat vier nieuwe high-advisories de audit-gate rood maakten). Beide sessies zitten erin (ADR 0058–0063). Daarna de Flexbuddy-rolwissel-bug framework-kant opgelost: Redis-adapter-key per omgeving afgeleid (`resolveSocketAdapterKey`, geen instelling) en sync-recipients vóór `_server` opgehaald zodat een adapter-timeout of lege room nooit meer een opgeslagen mutatie als fout meldt [ADR 0063]; unit 2142/2142, adapter-integratie 8/8, niet gecommit. (DEV-376-handoff: lege room is `sync.noReceiversFound` op elke transport [ADR 0058], lidmaatschapscheck per transport vastgelegd + dev-warning [0059], formatter-`userId` is context [0060], lokale socket-enumeratie onbereikbaar via lintregel + `getRoomSockets` + dev-Proxy op `getIoInstance()` [0061], GET-payload in de query-string vastgelegd [0062]; `SYNC_ERROR_CODES` geëxporteerd, 7 ontbrekende scaffold-locale-sleutels; unit 2132/2132; lockfile nog niet bijgewerkt). Daarvoor **v0.9.2 gepubliceerd** (17/17 op npm geverifieerd, tarball-grep bevestigt de fix; e2e-verdaccio 9/9 lokaal groen vóór de tag): een sync-route zonder de optionele `_client` telde als degraded type extraction, waardoor de enige manier om `fallbackCount` op nul te krijgen het schrijven van de no-op handlers was die de sync-architectuur verbiedt (ADR 0057; 44/46 meldingen bij Flexbuddy, 6→1 hier). Daarvoor **v0.9.1 gepubliceerd** (patch: de `ai:lint` escape-hatch + de verzoende framework-docs die via `framework-docs/` meereizen); 17/17 op npm geverifieerd, `ci` groen 4/4 vóór de tag. Daarvoor **v0.9.0 gepubliceerd** via GitHub Actions met SLSA-provenance; 17/17 packages op npm geverifieerd, CI op main groen. AI-context-artifacts zijn nu een gitignored cache met één `ai:refresh`, hook draait alleen checks, `ai:check-ids` blokkeert dubbele record-nummers. Twee stille cross-platform bugs gevangen die alleen de pipeline kon zien: `npx create-luckystack-app` deed niets op macOS/Linux (lesson 0021) en de frontmatter-parser was blind voor CRLF (lesson 0022). Daarna de vier stale docs verzoend met de code en de `ai:lint` escape-hatch gefixt, die alleen op dezelfde regel matchte en dus nooit werkte waar auteurs hem zetten. | 91 |
