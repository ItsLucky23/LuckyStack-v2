...
2. **`readSession(token)` → resolve `user`.** Reads the session via `@luckystack/core`'s session-provider registry (`readSession`). Preferred locale is also extracted here from `x-language` / `accept-language` headers using `extractLanguageFromHeader`. Error-tracker identity is propagated to registered trackers via `setCurrentErrorTrackerIdentity` (ALS-based per-request scope opened by `runWithErrorTrackerIdentityScope`) — NOT a direct `setSentryUser` call; that moved to hook subscribers in `@luckystack/error-tracking` (`preApiExecute` / `preApiValidate`).
...
4. **Built-in `system/logout` shortcut (socket only).** When the parsed route normalizes to `system/logout`, the handler applies the global per-IP rate-limit bucket, then calls `performLogout({ token, socket, userId })` and short-circuits with a `{ status: 'success', httpStatus: 200, result: true }` envelope routed through `emitApiResult` (so `preApiRespond`, `transformApiResponse`, and `postApiRespond` still fire). **Hooks that are skipped for logout:** `checkApiAuth` (no auth gate), `preApiValidate`, `postApiValidate`, `preApiExecute`, `postApiExecute`. HTTP transport does not have this shortcut — logout over HTTP goes through the normal route execution.
...
11. **`executeApiHandler(...)`.** Wraps `apiEntry.main({ data, user, functions, stream })` in `tryCatch` (which captures any thrown error to the active `@luckystack/error-tracking` adapter). Records `durationMs`. Span open/close and user-identity propagation are handled by hook subscribers registered in `@luckystack/error-tracking` (via `preApiExecute` / `postApiExecute`) — NOT by a direct `startSpan` or `setSentryUser` call inside this handler.
