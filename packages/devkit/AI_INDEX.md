...
- **Hot-reloading non-LuckyStack code.** The watchers know about LuckyStack's marker segments (`_api/`, `_sync/`) and the configured `srcDir` / `sharedDir` / `serverFunctionsDir`. Generic file watching belongs in chokidar directly.
...
- `projectConfig.paths.serverFunctionsDir` — root for `initializeFunctions()` + the functions watcher.
