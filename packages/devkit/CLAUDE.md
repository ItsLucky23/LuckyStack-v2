...
| `initializeFunctions()` | Clears `devFunctions`, resolves the module list via `collectFunctionModules()`, imports each one, and builds a nested `Record<string, unknown>` mirror of the on-disk tree from its key path. Dev passes `onConflict` so a duplicate warns + keeps the first claim instead of throwing. | -> docs/loader-pipeline.md |
| `collectFunctionModules(options?)` | **Single source of truth for function discovery** (ADR 0046). Walks every configured `serverFunctionDirs` root (default `['functions', 'shared']`, legacy singular `serverFunctionsDir` still honored) and derives each module's `functions.*` key path from its location relative to its root — so `shared/rbac/engine.ts` is `functions.rbac.engine`. Skips `node_modules`, test files (`.tests.`/`.test.`/`.spec.` via core's `isTestFile`), `__tests__`/`__mocks__` folders, and paths the `RoutingRules.ignore` predicate rejects. Throws on a cross-root duplicate key or a module/namespace collision; pass `onConflict` to warn + skip instead (dev-loader mode). `roots` overrides the configured roots for tests. Consumed by the dev loader, `functionsMeta.ts`, and BOTH `scripts/generateServerRequests.ts` copies. | -> docs/loader-pipeline.md |
| `renderFunctionsMap({ modules, importPrefix, varPrefix? })` | Renders discovered modules into the `functions` export of a generated production runtime-map file: `{ imports, source }`, with a NESTED object literal matching what the dev loader builds and `apiTypes.generated.ts` declares. A default-only export is aliased to the file name (`functions.sleep.sleep`). | -> docs/loader-pipeline.md |
| `formatFunctionKeyPath(keyPath)` / `functionModuleFileName(keyPath)` / `formatFunctionKeyConflict(...)` / `formatFunctionNamespaceConflict(...)` | Key-path helpers + the two canonical collision diagnostics, shared so dev and build-time emit identical wording. | -> docs/loader-pipeline.md |
| Types: `FunctionModule`, `RenderedFunctionsMap`, `CollectFunctionModulesOptions` | Public typing for the function registry. | -> docs/loader-pipeline.md |
...
| `typeMap/functionsMeta.ts` | Builds the `Functions` interface text emitted into `apiTypes.generated.ts`. Discovery comes from `collectFunctionModules()`; this module only parses each file's exports and places it at its key path. | -> docs/type-map-generation.md |
...
One case is deliberately NOT a fallback: a sync side that does not exist on disk. `_client` is optional by contract (`/docs/ARCHITECTURE_SYNC.md` — a pass-through sync must not have one, because a no-op handler costs an execution per recipient), and an absent side yields the same `{ }` text as a present side that declares no shape. The generator therefore carries per-entry presence flags so the emitter can tell the two apart, and suppresses only the field belonging to the missing side. A THROWN extraction is still checked first and always reported, so this can never hide a real failure — do not widen the exception past the missing side's own field.
...
- **Optional peer**: `@prisma/client@^6.19.0` — required only when consumer route/config types actually reference Prisma models; non-Prisma projects generate without it.
