...
| `attachSocketRedisAdapter(io: SocketIOServer): void` | Wire `@socket.io/redis-adapter` with duplicated pub/sub clients so room broadcasts span instances. The adapter `key` defaults to `resolveSocketAdapterKey()`. | -> docs/redis-adapter.md |
| `resolveSocketAdapterKey(): string` | The Redis-adapter channel key: `<project>:<envKey>:socket.io`. Instances on the same key + same Redis are ONE cluster; the env part keeps a dev laptop and a staging deploy that share a Redis server apart. | -> docs/redis-adapter.md |
| `setIoInstance(io: SocketIOServer \| null): void` | Module-level slot for the running Socket.io server. | -> docs/socket-bootstrap.md |
| `getIoInstance(options?: { raw?: boolean }): SocketIOServer \| null` | Read the slot from framework code that needs to broadcast. Outside production returns a guarded view that THROWS on `sockets.adapter.rooms` / `adapter.sids` and on enumerating `sockets.sockets` (per-instance maps; `.get(id)` allowed). `{ raw: true }` = explicit opt-in for deliberate per-instance work. Production always returns the raw instance. | -> docs/socket-bootstrap.md |
| `getRoomSockets(room, { userId? }): Promise<RemoteSocket[]>` | Cross-instance room enumeration: formats `room` under `'broadcast'`, then `io.in(...).fetchSockets()`; `'all'` = every socket everywhere. Throws when no Socket.io server is registered (never a silent `[]`). Use instead of the local adapter maps. | -> docs/socket-bootstrap.md |
...
| `MiddlewareInput`, `MiddlewareResult`, `MiddlewareOutcome`, `MiddlewareHandler`, `PageMiddleware` (types) | Route-guard contract types. Both handler shapes share the same signature. `MiddlewareResult` has three deny-capable shapes: allow (`{ success: true }`), redirect (`{ success: false, redirect }`) and deny-in-place (`{ success: false, status }` — keeps the URL, renders a status state). | -> docs/hooks.md |
| `resolveMiddlewareOutcome(result): MiddlewareOutcome` | Normalizes a middleware result to `allow` / `redirect` / `deny` / `fallback`. THE single interpretation, shared by `<Middleware>` and `useRouter` so programmatic navigation and direct URL hits cannot diverge. | -> docs/hooks.md |
...
| `getCsrfToken(origin?)`, `clearCsrfToken()`, `httpFetch(...)` | Origin-scoped CSRF-aware fetch wrapper; routed token-mode calls also attach bearer auth. | -> docs/socket-bootstrap.md |
| `invokeRoutedHttp(...)` (`/client`) | Browser-safe JSON/SSE invocation primitive with streaming, timeout and composed AbortSignals; used by typed API/sync routed mode. | -> docs/socket-bootstrap.md |
...
`@luckystack/core/eslint` exposes the shared ESLint rule set (the CLAUDE.md-invariant rules). Requires the optional `eslint@^9.0.0` peer. Includes `luckystack/no-local-socket-enumeration` (error when `@luckystack/core` or `@luckystack/sync` is installed): flags `.adapter.rooms` / `.adapter.sids` and enumeration of `.sockets.sockets` (`values`/`keys`/`entries`/`forEach`/`size`, `for…of`, spread) — per-instance maps that miss the rest of the cluster; `.get(id)` is allowed. Opt out per site with `// eslint-disable-next-line luckystack/no-local-socket-enumeration -- <why>`; the cross-instance replacement is `getRoomSockets()`.
