...
| `SYNC_ERROR_CODES` (readonly array) + type `SyncErrorCode` | Canonical list of every `sync.*` error code the framework can emit (both transports + the server's HTTP sync route). Also exported from `/client`. Write a locale-parity test against it instead of retyping a doc table. | → `docs/error-states.md` |
...
| `SYNC_ERROR_CODES` + type `SyncErrorCode` | Same canonical `sync.*` code list as the server barrel, for client-side locale parity tests and exhaustive `switch`es. | → `docs/error-states.md` |
...
| `preSyncRecipient` | Per recipient (both transports, remote recipients included), before that ONE socket receives the payload. Carries `{ routeName, receiver, recipientSocketId, recipientToken, recipientUserId, serverOutput }`. A stop signal SKIPS just that recipient (the loop continues; it is not counted as delivered) — it does NOT abort the whole fanout; a stop WITH `overrideOutput` delivers the override instead. `recipientToken` is the recipient's session token (`null` for an anonymous socket); `recipientUserId` is ALWAYS null — there is no resolver option, read the session by `recipientToken` yourself when needed. | → `docs/room-fanout.md` |
...
- `sync.allowClientReceiverAll` — when `false`, a client requesting the broadcast receiver `'all'` is rejected (`sync.receiverNotAllowed`, 403) on both transports (SYNC-07). Listed in `docs/error-states.md` §3. **Default `false` (0.2.0 secure-default flip — was `true`); opt back into cluster-wide broadcast explicitly or approve via `preSyncAuthorize`.**
- `sync.requireRoomMembership` — when `true`, a client may only target a room it is a member of; otherwise rejected (`sync.notRoomMember`, 403; listed in `docs/error-states.md` §3). Enforced on BOTH transports but from DIFFERENT sources, deliberately: the SOCKET path tests PHYSICAL membership (`socket.rooms` of the originating socket), the HTTP/SSE path tests LOGICAL membership (the session's persisted `roomCodes` + its own token-room) because it has no originator socket. Consequence: on `routed-http` a successful `syncRequest` does not prove you are in the room yourself or that you will receive the fan-out (dev-mode warning when this happens) — see `/docs/ARCHITECTURE_SYNC.md` → "Room membership is transport-specific". An anonymous HTTP caller (no session) has undeterminable membership and is rejected (fail-closed). **Default `true` (0.2.0 secure-default flip — was `false`); set `false` for the legacy any-room behavior.**
- Empty target room → `sync.noReceiversFound` (404) on BOTH transports since 0.10.0 (the HTTP path used to answer `success` with zero recipients). Recipients are resolved BEFORE `_server` runs on both transports, so this error — and a `fetchSockets()` adapter timeout (`sync.serverExecutionFailed`) — means nothing was persisted; a retry is safe. See `docs/room-fanout.md` §1.
