...
export const AUTH_SERVER_HOOKS = `import { registerHook, isProductionRuntime } from '@luckystack/core';
...
//? step with `assets/login/src/settings/_api/`. Exported for the ORM switcher
//? (switching a prisma+auth project away flags these as will-stop-compiling).
export const PRISMA_BOUND_SETTINGS_ROUTES = [
...
const DRIZZLE_USER_ADAPTER_STARTER = `//? Custom UserAdapter starter — data layer: DRIZZLE (generated by LuckyStack tooling).
...
const MIKRO_USER_ADAPTER_STARTER = `//? Custom UserAdapter starter — data layer: MIKRO-ORM (generated by LuckyStack tooling).
//? The built-in login UserAdapter is Prisma-backed; this file wires auth to
...
const NONE_USER_ADAPTER_STARTER = `//? Custom UserAdapter starter — data layer: NONE (generated by LuckyStack tooling).
...
//? Exported for the scaffolder-parity test: create-luckystack-app ships the
//? SAME drizzle/mikro starters at scaffold time (byte-identical, guarded).
export const USER_ADAPTER_STARTERS: Record<string, string> = {
...
//? When the data layer is not Prisma, write the matching starter adapter +
//? spell out what is skipped. Non-blocking: a finished custom adapter is a
//? fully supported setup.
const adaptAuthToDataLayer = (project: ConsumerProject, orm: string): void => {
  ...
  console.warn(
    `\n⚠ Data layer is '${orm}' — the built-in login UserAdapter is Prisma-backed.\n` +
      '  Finish luckystack/login/userAdapter.ts (starter written above; create the users table + uncomment)\n' +
      '  before sign-in works. The Prisma-bound settings routes + notification hooks are NOT copied\n' +
      '  on this data layer (they call functions.db.prisma / getPrismaClient() directly):\n' +
      [...PRISMA_BOUND_SETTINGS_ROUTES, 'server/hooks/notifications.ts'].map((route) => `       - ${route}`).join('\n') +
      '\n  Full contract + examples: node_modules/@luckystack/login/docs/user-adapter.md\n',
  );
};

//? The asset-bundle paths that are Prisma-bound (see adaptAuthToDataLayer) —
//? on a non-Prisma layer they are removed from the copy again so `add login`
//? never leaves the consumer with code that cannot compile.
const isPrismaBoundAssetPath = (root: string, absolutePath: string): boolean => {
  const relative = path.relative(root, absolutePath).replaceAll('\\', '/');
  return relative.startsWith('src/settings/') || relative === 'server/hooks/notifications.ts';
};

//? Remove a directory tree that holds no files anymore (leftover empty
//? src/settings/_api shells after the withdraw above). Consumer files keep
//? the tree alive — only truly-empty directories go.
const removeDirIfEmptyDeep = (dir: string): void => {
  if (!fs.existsSync(dir)) return;
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (entry.isDirectory()) removeDirIfEmptyDeep(path.join(dir, entry.name));
  }
...
export const addLogin = (project: ConsumerProject, options: AddOptions): Result<void> => {
  ...
  adaptAuthToDataLayer(project, orm);
  ...
  //? Non-prisma: withdraw the Prisma-bound files this call JUST wrote (only
  //? just-written ones — skip-if-exists means pre-existing consumer files were
  //? never in `written`, so consumer edits are never deleted here).
  if (orm !== 'prisma') {
    const withdrawn = written.filter((file) => isPrismaBoundAssetPath(project.root, file));
    for (const file of withdrawn) fs.rmSync(file, { force: true });
    if (withdrawn.length > 0) removeDirIfEmptyDeep(path.join(project.root, 'src', 'settings'));
    written = written.filter((file) => !withdrawn.includes(file));
  }
  if (written.length > 0) {
    console.log(`• copied ${String(written.length)} auth file(s) into the project (pages + _api + LoginForm + functions/session.ts${orm === 'prisma' ? ' + server/hooks/notifications.ts' : ' — Prisma-bound settings/notifications skipped'})`);
  } else {
  ...
  //? Register the notification hooks in the server overlay (reverse of the
  //? prune) — prisma only: the hooks read the user via getPrismaClient(), and
  //? on another data layer the hooks file itself was withdrawn above.
  const hooksWired = orm === 'prisma' && tryEdit(
