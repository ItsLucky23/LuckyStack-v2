...
describe('planChanges — toggles', () => {
  it('presence off → on', () => {
    expect(summaries(cfg(), cfg({ toggles: { presence: true, sync: false, 'docs-ui': false } }))).toEqual(['presence: off → on']);
  });
  it('docs-ui on → off', () => {
    const from = cfg({ toggles: { presence: false, sync: false, 'docs-ui': true } });
...
describe('planChanges — combined', () => {
  it('orders auth, email, monitoring, toggles', () => {
    const to = cfg({ authMode: 'credentials', email: 'resend', monitoring: 'sentry', toggles: { presence: true, sync: false, 'docs-ui': false } });
