...
`create-luckystack-app` is the interactive scaffold CLI for LuckyStack projects. Consumers invoke it via `npx create-luckystack-app <project-name>`; it copies the bundled `template/` tree into a fresh target directory, substitutes a small set of template variables, optionally prompts for high-level scaffold choices (ORM/database provider, package manager, auth mode, OAuth providers, email adapter, monitoring backend, optional packages), and runs the selected package-manager install plus the relevant ORM generation step so the project is buildable on first try. i18n (the multi-language setup: nl/de/fr locales + switcher) ALWAYS ships — the translator is core, so there's no toggle for it.
...
Ports have a single source of truth: `template/config.ports.ts` (`export const ports = { frontend, backend }`) ALWAYS ships — `config.ts` re-exports it, `server.ts` passes `ports.backend` to the server as `defaultPort`, and `vite.config.ts` reads `ports.frontend`. `viteBackendProxy.ts` updates both Vite's request clone and the original node-http-proxy options so HTTP + WebSocket traffic follows `node_modules/.luckystack/dev-server.json` after a hop. `scripts/testAll.ts` uses `@luckystack/test-runner`'s same live-port resolver with `ports.backend` as fallback. The scaffold has no backend listen-port env key (a positional argv `<port>` overrides for multi-instance boots; `SERVER_IP` stays in `.env`).
...
- `--pm=<npm|bun>` — package manager for the post-scaffold install (default `npm`). **npm + bun ONLY** — pnpm/yarn are deliberately not offered (`--pm=pnpm` exits 2). `bun` must already be on `PATH` (not found → install skipped + a hint, never a crash). `--pm=bun` also writes `"packageManager": "bun@<BUN_VERSION_FLOOR>"` into the rendered package.json — the field `@luckystack/cli`'s `detectPackageManager` reads, so every later `luckystack add`/`remove`/`manage` install uses bun too. The choice lands in `.luckystack/scaffold.json` so `update` re-renders with it. **`prisma generate` deliberately stays on `npx` even under bun** — Prisma's CLI is a Node program (`bunx` just defers to it via the shebang), and `bunx --bun prisma generate`, the only variant that truly swaps runtime, has an open Windows infinite-hang (oven-sh/bun#14868); Prisma's own docs scope `--bun` to `prisma init`. See the comment on `runPrismaGenerate`.
