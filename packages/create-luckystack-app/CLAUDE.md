...
| Constants: `PRISMA_USER_TYPE_IMPORT` / `ORM_NONE_CONFIG_USER_TYPE` / `DRIZZLE_DRIVER_DEPS` / `MIKRO_DRIVER_PACKAGES` | Exported ORM-surface tokens (the config.ts `User`-type import + orm-none placeholder block, and the per-db driver dep tables). Consumed by the CLI's `switchOrm.test.ts` parity guards so the manage-wizard ORM switch can never drift from what the scaffolder actually renders. | (module header) |
...
- `--orm=<prisma|drizzle|mikro-orm|none>` — data layer (default `prisma`; ADR 0020). Every non-prisma value strips the Prisma surface (prisma/ dir, `@prisma/client` + `prisma` deps, `prisma:*` scripts, `scripts/prismaWithSecrets.ts`), swaps config.ts's Prisma `User` type import for a local placeholder, FORCES `--auth=none` (the built-in UserAdapter is Prisma-backed — a custom UserAdapter re-enables auth), and skips `prisma generate`. Then per value:
