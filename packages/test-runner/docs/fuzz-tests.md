...
| `{ constructor: { polluted: true } }` | Prototype-pollution attempt via a named own-key. `{ __proto__: ... }` serializes to `{}` via JSON.stringify (prototype keys are not own-enumerable), so the probe uses `constructor` instead — it survives the JSON round-trip while still exercising parsers that misinterpret the key. |
...
- **Prototype pollution.** The runner sends `{ constructor: { polluted: true } }`. `{ __proto__: ... }` serializes to `{}` via JSON.stringify (prototype keys are not own-enumerable), so the probe uses `constructor` instead — it survives the JSON round-trip while still exercising parsers that misinterpret the key. A failure here means a handler is using a custom merge function that conflates `constructor` with `Object.prototype.constructor`.
