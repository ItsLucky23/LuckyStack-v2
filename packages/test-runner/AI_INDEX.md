...
Generated-type-driven test layers voor LuckyStack APIs. Walkt elke endpoint uit de generated `apiMethodMap` en draait vier progressive layers: contract smoke (Zod-valid input), auth enforcement, rate-limit en crash-resistance fuzz. Elke layer is independent runnable. Reporter/fixture/layer-extension registries laten consumers eigen checks plug-in zonder de framework te forken.
...
- Voor sync-event testing — de runner walkt momenteel alleen API endpoints, niet de sync map (sync support staat op de roadmap maar is nog niet geleverd).
...
- Devkit (genereert de maps die deze runner consumeert): `packages/devkit/AI_INDEX.md`.
- Server (`/_test/reset` endpoint): `packages/server/AI_INDEX.md`.
