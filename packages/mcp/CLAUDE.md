...
- The artifacts it serves are produced by `scripts/generateGraph.mjs` (ai:graph), `generateDecisionsIndex.mjs` (ai:decisions), `generateProjectIndex.mjs` (ai:project-index), `generateLessonsIndex.mjs` (ai:lessons), `generateAiCapabilities.mjs` (ai:capabilities).
