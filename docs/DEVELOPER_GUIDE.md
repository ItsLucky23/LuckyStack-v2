...
**How the client follows the backend port (dev).** In dev the browser talks to the app **same-origin**: Vite serves the frontend on `ports.frontend` and **proxies** `/api`, `/sync`, `/auth`, `/socket.io`, … to the backend — so the client never needs to know the backend port. If `ports.backend` is already in use, the server **auto-increments** to the next free port (`SERVER_PORT_AUTO_INCREMENT`, default ON in dev) and writes the *actually-bound* port to `node_modules/.luckystack/dev-server.json`. The Vite proxy updates both its HTTP and WebSocket upstream per request, and `npm test` resolves the same advertisement before falling back to `ports.backend`. Port-file cleanup is PID-owned, so an old process cannot erase a newer process's advertisement.

For OAuth, an auto-derived direct loopback callback follows the same intended → bound hop on both authorize and token exchange. An explicitly configured localhost router/reverse-proxy callback on another port remains authoritative; the framework does not bypass that ingress.
