...
> **The `listen()` boundary decides what is fatal.** Anything that fails *before* the server listens kills the process — a failure there means there is no server, so the scaffold's "failed to start" message is always literally true. Work handed to `server.afterListen(task)` runs once the server is already accepting traffic, and a failure there is logged and the process keeps serving; pass `{ fatal: true }` for a task the process genuinely cannot run without (ADR 0050). This matters most behind a supervisor: without the split, one unreachable dependency of a post-listen task turns a healthy server into a crash-loop that restarts, fails on the same dependency, and reports "failed to start" the whole time. So when you triage: a crash-loop points at boot wiring; a single post-listen error means a background task failed on a server that is still serving traffic.
>
