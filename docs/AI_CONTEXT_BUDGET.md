...
## Artifact sizes (total ≈ 75,681 tokens if all loaded)

| Artifact | Est. tokens | Bytes | Purpose |
| --- | --- | --- | --- |
| `contract` — `CLAUDE.md` | 10,693 | 42,773 | Always-on rules (read every session). |
| `quick-index` — `docs/AI_QUICK_INDEX.md` | 32,208 | 128,833 | Framework surfaces + package map. |
...
| `decisions` — `docs/AI_DECISIONS_INDEX.md` | 7,740 | 30,961 | Why-it-is-this-way (ADRs). |
| `lessons` — `docs/AI_LESSONS_INDEX.md` | 1,405 | 5,619 | Known pitfalls (what failed). |
...
- **Load** (≈ 16,382 tokens): `contract`, `capabilities`, `examples`, `runbooks`
...
- **Load** (≈ 15,074 tokens): `contract`, `project-index`, `examples`, `product`
...
- **Load** (≈ 21,116 tokens): `contract`, `decisions`, `project-index`
...
- **Load** (≈ 12,098 tokens): `contract`, `lessons`
