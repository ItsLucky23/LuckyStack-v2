...
## Artifact sizes (total ≈ 88,001 tokens if all loaded)

| Artifact | Est. tokens | Bytes | Purpose |
| --- | --- | --- | --- |
| `contract` — `CLAUDE.md` | 11,580 | 46,320 | Always-on rules (read every session). |
| `quick-index` — `docs/AI_QUICK_INDEX.md` | 34,939 | 139,756 | Framework surfaces + package map. |
| `capabilities` — `docs/AI_CAPABILITIES.md` | 3,782 | 15,127 | Existing helpers/exports — check before authoring. |
| `project-index` — `docs/AI_PROJECT_INDEX.md` | 2,683 | 10,733 | Routes/pages/helpers/components + test coverage. |
| `decisions` — `docs/AI_DECISIONS_INDEX.md` | 12,081 | 48,322 | Why-it-is-this-way (ADRs). |
| `lessons` — `docs/AI_LESSONS_INDEX.md` | 5,727 | 22,906 | Known pitfalls (what failed). |
...
- **Load** (≈ 17,309 tokens): `contract`, `capabilities`, `examples`, `runbooks`
...
- **Load** (≈ 15,961 tokens): `contract`, `project-index`, `examples`, `product`
...
- **Load** (≈ 26,344 tokens): `contract`, `decisions`, `project-index`
...
- **Load** (≈ 17,307 tokens): `contract`, `lessons`
