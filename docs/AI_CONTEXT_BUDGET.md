...
## Artifact sizes (total ≈ 80,061 tokens if all loaded)

| Artifact | Est. tokens | Bytes | Purpose |
| --- | --- | --- | --- |
| `contract` — `CLAUDE.md` | 11,579 | 46,314 | Always-on rules (read every session). |
| `quick-index` — `docs/AI_QUICK_INDEX.md` | 33,217 | 132,868 | Framework surfaces + package map. |
...
| `decisions` — `docs/AI_DECISIONS_INDEX.md` | 8,310 | 33,241 | Why-it-is-this-way (ADRs). |
| `lessons` — `docs/AI_LESSONS_INDEX.md` | 3,321 | 13,282 | Known pitfalls (what failed). |
...
- **Load** (≈ 22,572 tokens): `contract`, `decisions`, `project-index`
...
- **Load** (≈ 14,900 tokens): `contract`, `lessons`
