...
| CI-02 | A Windows-specific Bun-supervisor test used the host OS's `path.isAbsolute`, so its intentionally Windows-shaped fixture failed on Linux after the lockfile let CI finally reach the unit suite. | MED | in-progress | 2026-07-16 | — | Runtime code was not implicated: the other nine supervisor tests passed. The assertion now uses `path.win32.isAbsolute`, matching the Windows `.cmd` scenario it explicitly simulates. Awaiting corrected Linux CI. |
