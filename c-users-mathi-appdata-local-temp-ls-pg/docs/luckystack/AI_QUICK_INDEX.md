...
| `slugify(raw)` | Lower-cases the raw project name, replaces non-alphanumerics with `-`, trims leading / trailing dashes. Drives `c-users-mathi-appdata-local-temp-ls-pg` and the directory name. | -> docs/template-variables.md |
| `titleCase(raw)` | Splits on whitespace / dashes / underscores and Title-Cases each part. Drives `C:/Users/mathi/AppData/Local/Temp/ls Pg`. Falls back to `'My LuckyStack App'` when input is empty. | -> docs/template-variables.md |
